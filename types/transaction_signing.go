...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int, blockTime uint64) Signer {
	var signer Signer
	switch {
	case config.IsCancun(new(big.Int).SetUint64(blockTime), blockNumber):
...
type Signer interface {
	...
	Hash(tx *Transaction) ctypes.Hash
...
func (s pragueSigner) SignatureValues(tx *Transaction, sig []byte) (R, S, V *big.Int, err error) {
	...
	if txdata.ChainID < 0 && txdata.ChainID != s.chainId.Uint64() {
...
func (s pragueSigner) Hash(tx *Transaction) ctypes.Hash {
	if tx.Type() != SetCodeTxType {
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s cancunSigner) Hash(tx *Transaction) ctypes.Hash {
	if tx.Type() != BlobTxType {
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s londonSigner) Hash(tx *Transaction) ctypes.Hash {
	if tx.Type() != DynamicFeeTxType {
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s eip2930Signer) Hash(tx *Transaction) ctypes.Hash {
	switch tx.Type() {
	case LegacyTxType:
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s EIP155Signer) Hash(tx *Transaction) ctypes.Hash {
	return rlpHash([]interface{}{
		tx.Nonce(),
		tx.GasPrice(),
		tx.Gas(),
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (fs FrontierSigner) Hash(tx *Transaction) ctypes.Hash {
	return rlpHash([]interface{}{
		tx.Nonce(),
		tx.GasPrice(),
		tx.Gas(),
...
func recoverPlain(sighash ctypes.Hash, R, S, Vb *big.Int, homestead bool) (common.Address, error) {
	if Vb.BitLen() > 8 {
