...
func init() {
	...
	type ctorFn = func(*tracers.Context, json.RawMessage) (vm.Tracer, error)
	lookup := func(code string) ctorFn {
		return func(ctx *tracers.Context, cfg json.RawMessage) (vm.Tracer, error) {
...
func newJsTracer(code string, ctx *tracers.Context, cfg json.RawMessage) (vm.Tracer, error) {
	vm := goja.New()
...
func (mo *memoryObj) slice(begin, end int64) ([]byte, error) {
	...
	slice, err := tracers.GetMemoryCopyPadded(mo.memory.Data(), begin, end-begin)
	if err != nil {
		return nil, err
	}
	return slice, nil
