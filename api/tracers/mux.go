...
func init() {
	...
	Registry["muxTracer"] = newMuxTracer
}
...
func newMuxTracer(ctx *Context, cfg json.RawMessage) (TracerResult, error) {
	...
	objects := make([]TracerResult, 0, len(config))
	...
	for k, v := range config {
		if fn, ok := Registry[k]; ok {
			...
			return nil, fmt.Errorf("tracer not found: %v", k)
		}
	}
...
func (t *muxTracer) CaptureEnd(output []byte, gasUsed uint64, d time.Duration, err error) {
	for _, t := range t.tracers {
		t.CaptureEnd(output, gasUsed, d, err)
	}
}
...
func (t *muxTracer) Result() interface{} {
	result := make([]interface{}, len(t.tracers))
	for i, tt := range t.tracers {
		result[i] = tt.Result()
	}
	return result
