...
func init() {
	Registry["flatCallTracer"] = newFlatCallTracer
}
...
func newFlatCallTracer(ctx *Context, cfg json.RawMessage) (TracerResult, error) {
	...
	tracer, err := Registry["callTracer"](ctx, cfg)
	if err != nil {
		return nil, err
	}
...
func (t *flatCallTracer) CaptureEnd(output []byte, gasUsed uint64, d time.Duration, err error) {
	t.tracer.CaptureEnd(output, gasUsed, d, err)
}
...
func (t *flatCallTracer) Result() interface{} {
	v, _ := t.GetResult()
	return v
}
