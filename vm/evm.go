...
type (
	...
	GetHashFunc func(uint64) ctypes.Hash
)

func (evm *EVM) precompile(addr common.Address) (PrecompiledContract, bool) {
	var precompiles map[common.Address]PrecompiledContract
	switch {
	case evm.chainRules.IsPrague && evm.chainRules.IsNapoli:
		precompiles = PrecompiledContractsNapoliPrague
...
type TxContext struct {
	...
	BlobHashes []ctypes.Hash // Provides information for BLOBHASH
}
...
func NewEVM(blockCtx BlockContext, txCtx TxContext, statedb state.StateDB, chainConfig *params.ChainConfig, config Config) *EVM {
	if blockCtx.CanTransfer == nil {
		blockCtx.CanTransfer = defaultCanTransfer
	}
	if blockCtx.Transfer == nil {
		blockCtx.Transfer = defaultTransfer
	}
	evm := &EVM{
		Context:     blockCtx,
		TxContext:   txCtx,
...
func (c *codeAndHash) Hash() ctypes.Hash {
	if c.hash == (ctypes.Hash{}) {
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address, typ OpCode) ([]byte, common.Address, uint64, error) {
	...
	if evm.StateDB.GetNonce(address) != 0 || (contractHash != (ctypes.Hash{}) && contractHash != emptyCodeHash) {
	...
	if err == nil && evm.chainRules.IsEIP158 {
		if evm.chainRules.IsPip30 && len(ret) > params.MaxCodeSizePIP30 {
...
func (evm *EVM) resolveCodeHash(addr common.Address) ctypes.Hash {
	if evm.chainRules.IsPrague {
		code := evm.StateDB.GetCode(addr)
		if target, ok := types.ParseDelegation(code); ok {
			// Note we only follow one level of delegation.
