...
type accessListSlots map[ctypes.Hash]struct{}
...
func (al accessList) addAddress(address common.Address) {
	...
	if _, present := al[address]; !present {
		al[address] = make(map[ctypes.Hash]struct{})
	}
}

// addSlot adds a storage slot to the accesslist.
func (al accessList) addSlot(address common.Address, slot ctypes.Hash) {
	// Set address if not previously present
...
func (al accessList) accessList() types.AccessList {
	...
	for addr, slots := range al {
		tuple := types.AccessTuple{Address: addr, StorageKeys: []ctypes.Hash{}}
...
func (a *AccessListTracer) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, scope *ScopeContext, rData []byte, depth int, err error) {
	...
	if (op == SLOAD || op == SSTORE) && stack.len() >= 1 {
		slot := ctypes.Hash(stack.data[stack.len()-1].Bytes32())
