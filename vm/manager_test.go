...
func TestViewHash(t *testing.T) {
	...
	err := mgr.View(func(h types.Hash) {
		if h != types.HexToHash("a") {
			t.Errorf("Unexpected hash value: %#x", h.Bytes())
		}
	})
	if err != nil {
		t.Errorf(err.Error())
...
func TestViewUnexpectedInput(t *testing.T) {
	...
	if err == nil {
		t.Errorf("Expected unknown parameter error")
	}
}
...
func TestViewBlockNumber(t *testing.T) {
	...
	err := mgr.View(func(num uint64) {
		if num != 1 {
			t.Errorf("Unexpected block number")
		}
	})
	if err != nil {
		t.Errorf(err.Error())
...
func TestViewTx(t *testing.T) {
	...
	err := mgr.View(func(hash types.Hash, tx storage.Transaction) {
		if tx.HashToNumber(hash) != 1 {
			t.Errorf("Unexpected hash number")
		}
	})
	if err != nil {
		t.Errorf(err.Error())
...
func TestViewHeader(t *testing.T) {
	...
	err := mgr.View(types.HexToHash("a"), func(h types.Hash, header *etypes.Header) {
		if header.GasLimit != 15000000 {
			t.Errorf("Unexpected gas limit")
		}
...
func TestStatedb(t *testing.T) {
	...
	err := mgr.View(context.Background(), func(state state.StateDB) {
		if state.GetNonce(common.Address{}) != 0 {
...
func TestViewEVM(t *testing.T) {
	...
	err := mgr.View(context.Background(), func(evm *EVM) {
		evm.Cancel()
		if !evm.Cancelled() {
			t.Errorf("EVM should have been cancelled")
		}
	})
	if err != nil {
		t.Errorf(err.Error())
